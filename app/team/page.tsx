...
export default async function TeamPage(){
  ...
  return <main className={layoutClass}><Header/><PageHero eyebrow={page.eyebrow} title={page.title.replaceAll("{{brand}}",brand)} subtitle={page.subtitle.replaceAll("{{brand}}",brand)}/>
    <section className="page-section"><div className="wrap"><span className="kicker">Current executive board</span><h2 className="section-heading">{pageCopy(page,"currentHeading","Building the next chapter.")}</h2>{pageCopy(page,"currentBody","")&&<p>{pageCopy(page,"currentBody","")}</p>}<div className="team-grid">{current.map(card)}</div>{!current.length&&<div className="event-empty"><h2>Board profiles are being updated.</h2><p>{pageCopy(page,"emptyState","In the meantime, contact the organization through the Join page.")}</p><Link className="btn red" href="/join">Join / Contact</Link></div>}</div></section>
    {!!past.length&&<section className="cream-section"><div className="wrap"><span className="kicker">Past boards</span><h2 className="section-heading">{pageCopy(page,"pastHeading","The students who built the community.")}</h2>{pageCopy(page,"pastBody","")&&<p>{pageCopy(page,"pastBody","")}</p>}{years.map(year=><section className="past-board-year" key={year}><h3>{year}</h3><div className="team-grid compact">{past.filter(m=>m.boardYear===year).map(card)}</div></section>)}</div></section>}
