...
export default async function GalleryPage(){
  ...
  return <main><Header/><PageHero eyebrow="Gallery" title="Memories that connect us." subtitle={`A growing archive of the people, events and experiences that make ${site.settings.shortName} what it is.`}/>
    {site.gallery.length>0&&<section className="page-section"><div className="wrap gallery-intro"><div><span className="kicker">Club media library</span><h2>Photos and videos managed by the board.</h2></div><p>Filter by year, album, or media type. Featured items are promoted across the site and are available to the newsletter builder.</p></div><div className="wrap"><GalleryBrowser items={site.gallery} eventTitles={eventTitles}/></div></section>}
    ...
    <section className="archive-section"><div className="wrap archive-head"><span className="kicker light">Club archive</span><h2>More events in the public record.</h2><p>These records are ready to become full event pages as the board adds photos and details.</p></div><div className="wrap archive-grid">{archiveEvents.map(item=><article key={item.title}><span className="kicker light">{item.date}</span><h3>{item.title}</h3><p>{item.source}</p></article>)}</div></section><Footer/></main>;
}
