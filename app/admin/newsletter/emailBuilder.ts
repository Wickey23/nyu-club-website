...
export type NewsletterSettings={clubName:string;shortName:string;email:string;instagram:string;linkedin:string};
...
function eventSection(events:NewsletterEvent[]){
  if(!events.length)return "";
  return `<tr><td style="padding:10px 32px 8px"><h2 style="font-family:Georgia,serif;font-size:28px;margin:0;color:#171412">Coming up</h2></td></tr>${events.map(eventCard).join("")}`;
}

function galleryCell(g:NewsletterGalleryItem){
  return `<td width="33.33%" style="padding:3px"><img src="${escapeHtml(galleryImageUrl(g.image))}" alt="${escapeHtml(g.title)}" width="180" style="width:100%;height:150px;object-fit:cover;display:block;border-radius:10px"></td>`;
}
...
function gallerySection(items:NewsletterGalleryItem[]){
  ...
  const rows=galleryRow(items.slice(0,3))+(items.length>3?galleryRow(items.slice(3,6)):"");
  return `<tr><td style="padding:22px 32px 8px"><h2 style="font-family:Georgia,serif;font-size:28px;margin:0 0 14px;color:#171412">From the community</h2><table role="presentation" width="100%" style="border-collapse:collapse">${rows}</table></td></tr>`;
}
