...
export default function AdminDashboard({ adminEmail }: { adminEmail:string }) {
  ...
  async function save() {
    ...
    const data=await response.json();
    setSaving(false);
    setMessage(response.ok ? "Saved. Vercel will publish the update automatically." : (data.error||"Save failed."));
  }
  ...
  async function upload(event:ChangeEvent<HTMLInputElement>, apply:(url:string)=>void) {
    ...
    apply(data.url); setMessage("Image uploaded. Click Save changes to publish it.");
  }
  ...
  return <main className="cms-shell">
    <aside className="cms-sidebar">
      ...
      <div className="cms-user"><span>Signed in as</span><b>{adminEmail}</b><button onClick={logout}>Sign out</button></div>
    </aside>

    <section className="cms-main">
      <header className="cms-top"><div><span className="admin-kicker">Content management</span><h1>{tab}</h1></div><div className="cms-actions"><a href="/" target="_blank">View site ↗</a><button onClick={save} disabled={saving} className="admin-primary">{saving?"Saving…":"Save changes"}</button></div></header>
