...
export default function PublicPagesEditor(){
 const[rows,setRows]=useState<PageRow[]>([]);const[selected,setSelected]=useState("about");const[message,setMessage]=useState("");const[saving,setSaving]=useState(false);const[loading,setLoading]=useState(true);const[previewKey,setPreviewKey]=useState(0);
 ...
 return <main className="public-pages-admin"><header className="public-pages-head"><div><span className="admin-kicker">Super Admin only</span><h1>Public Pages</h1><p>Edit the real public-facing copy and page presentation while seeing the site beside your controls.</p></div><div><a href="/admin">← Board CMS</a><button className="admin-primary" onClick={save} disabled={saving}>{saving?"Saving…":"Save & refresh preview"}</button></div></header>{message&&<div className="cms-notice">{message}</div>}
