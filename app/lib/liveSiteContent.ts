...
async function loadLiveSiteContent(): Promise<SiteContent> {
  ...
  try {
    const [homeResult,eventResult,teamResult,galleryResult,settingsResult] = await Promise.all([
      ...
      supabase.from("site_settings").select("club_name,short_name,email,instagram,linkedin,facebook,cms_config").eq("id",1).single(),
    ]);
    ...
    return {
      ...
      settings:{ clubName:settings.club_name,shortName:settings.short_name,email:settings.email,instagram:settings.instagram,linkedin:settings.linkedin,facebook:settings.facebook||"",cmsConfig:(settings.cms_config&&typeof settings.cms_config==="object"?settings.cms_config:{}) as Record<string,any> },
    };
  } catch {
    return {
      ...fallback,
      ...
      settings:{...fallback.settings,cmsConfig:fallback.settings.cmsConfig||{}},
