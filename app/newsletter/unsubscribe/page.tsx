...
export default function UnsubscribePage(){
  ...
  useEffect(()=>{
    ...
    void supabase.from("site_settings").select("short_name,club_name").eq("id",1).single().then(({data})=>{
    ...
    void supabase.rpc("unsubscribe_newsletter",{p_token:token}).then(({error})=>{
      if(error) setMessage("We could not update your subscription. Please contact the club.");
      else {
