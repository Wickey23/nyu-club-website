...
function googleCalendar(event:any){
  if(!event.date)return "";
  const date=event.date.replaceAll("-","");
  const times=event.time?`${date}T${event.time.replace(":","")}00/${date}T${(event.endTime||event.time).replace(":","")}00`:`${date}/${date}`;
  const q=new URLSearchParams({action:"TEMPLATE",text:event.title,dates:times,details:event.description||"",location:event.location||""});
...
export default async function EventDetail({params}:{params:Promise<{slug:string}>}){
  ...
  const event=site.events.find(e=>(e.slug||e.id)===slug||e.id===slug);
  if(!event)notFound();
  const related=site.gallery.filter(g=>g.eventId===event.id).slice(0,8);
  return <main><Header/>
