...
export async function generateMetadata():Promise<Metadata>{const site=await getLiveSiteContent();const formal=site.settings.clubName||"NYU Peruvian Student Association";return{title:`About | ${formal}`,description:"Learn about the NYU Peruvian Student Association, its mission, community, history and cultural programming at NYU."};}
...
export default async function AboutPage(){
  ...
  const page=await getPublicPageSettings("about",{eyebrow:"About us",title:"Rooted in culture. Built on community.",subtitle:`Meet ${brand} — NYU's Peruvian student community in New York City.`});
  ...
  return <main className={layoutClass}><Header/><PageHero eyebrow={page.eyebrow} title={page.title.replaceAll("{{brand}}",brand)} subtitle={page.subtitle.replaceAll("{{brand}}",brand)} kind="photo"/>
    <section className="page-section"><div className="wrap prose-grid"><div><span className="kicker">Who we are</span><h2>{pageCopy(page,"missionHeading","A home for Peru at NYU.")}</h2></div><div><p>{pageCopy(page,"missionBody","Founded in 2016, the NYU Peruvian Student Association celebrates Peruvian and Andean culture while creating an inclusive community for Peruvians, the diaspora, and anyone who wants to learn, connect and celebrate together.")}</p><p>Programming spans food, music, art, heritage, conversations, social events and collaborations across NYU and New York City.</p></div></div>
    ...
    <section className="page-section alt"><div className="wrap split-feature"><div className="photo-card sourced-photo" style={bg(clubMedia.conversation[1],"center 35%") }><span>Official club social photography</span></div><div><span className="kicker">Since 2016</span><h2>{pageCopy(page,"historyHeading","Culture is the starting point. Connection is the goal.")}</h2><p className="feature-body">{pageCopy(page,"historyBody","The organization has grown through student-led programs that bring people together around Peruvian identity, shared experiences and relationships that extend beyond a single event.")}</p><div className="about-fact-grid"><div><b>2016</b><span>Founded at NYU</span></div><div><b>{currentBoard.length||"—"}</b><span>Current board members</span></div><div><b>{years.length||"—"}</b><span>Board years in the archive</span></div></div><div className="actions"><Link className="btn red" href="/team">Meet the board</Link><Link className="btn outline" href="/join">Join the community</Link></div></div></div></section>
